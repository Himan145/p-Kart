...
export const ProductDetais=()=>{
    ...
    const getProduct=async()=>{
        try{
            const {data}=await axios.get(`https://p-kart.vercel.app/api/v1/product/get-single-product/${params.slug}`);
    ...
    const getSimilarProduct=async(pid,cid)=>{
        try{
            const {data}=await axios.get(`https://p-kart.vercel.app/api/v1/product/related-product/${pid}/${cid}`)
    ...
    return(<Layout title={"p-Kart ProductDetails"}>
        <div className='product-details'>
            <div className='row container mt-3'>
                ...
                <img src={`https://p-kart.vercel.app/api/v1/product/get-photo/${product?._id}`} className='card-img-top' alt={product?.name} height={"500px"} width={"500px"}/> 
                </div>
            ...
            <div className='row'>
                ...
                <div className='d-flex flex-wrap '>
                        {relatedProducts?.map((p)=>(
                            ...
                            <img src={`https://p-kart.vercel.app/api/v1/product/get-photo/${p._id}`} className='card-img-top' alt={p.name}/>
