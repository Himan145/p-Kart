...
export const HomePage=()=>{
    ...
    const getTotal=async()=>{
        try{
            const {data}=await axios.get('https://p-kart-backend.vercel.app/api/v1/product/product-count');
            setTotal(data?.total);
        }
        catch(err){
            console.log(err);
    ...
    const getAllProducts=async()=>{
        try{
            setLoading(true);
            const {data}=await axios.get(`https://p-kart-backend.vercel.app/api/v1/product/product-list/${page}`);
            setLoading(false);
    ...
    const getCategory=async()=>{
        try{
            const {data}=await axios.get('https://p-kart-backend.vercel.app/api/v1/category/get-category');
            if(data.success){
    ...
    const filterProduct = async () => {
        try {
          const { data } = await axios.post("https://p-kart-backend.vercel.app/api/v1/product/product-filter", {
            checked,
            radio,
          });
          setProducts(data?.products);
                ...
                <div className='col-md-10'>
                    ...
                    <div className='d-flex flex-wrap justify-content-evenly align-items-center'>
                        {products?.map((p)=>(
                            ...
                            <img src={`https://p-kart-backend.vercel.app/api/v1/product/get-photo/${p._id}`} className='card-img-top' alt={p.name}/>
