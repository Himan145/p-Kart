...
export const HomePage=()=>{
    ...
    const getTotal=async()=>{
        try{
            const {data}=await axios.get('https://p-kart.vercel.app/api/v1/product/product-count');
            setTotal(data?.total);
        }
        catch(err){
            console.log(err);
    ...
    const getAllProducts=async()=>{
        try{
            setLoading(true);
            const {data}=await axios.get(`https://p-kart.vercel.app/api/v1/product/product-list/${page}`);
            setLoading(false);
    ...
    const filterProduct = async () => {
        try {
          const { data } = await axios.post("https://p-kart.vercel.app/api/v1/product/product-filter", {
            checked,
            radio,
          });
          setProducts(data?.products);
